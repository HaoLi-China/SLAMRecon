...
#include "FEDepthTracker_CUDA.h"
#include "FECUDAUtils.h"
#include "../Common/FECDepthTracker.h"
#include "CUDADefines.h"

using namespace FE;

struct FEDepthTracker_CUDA::AccuCell {
	int numPoints;
	float f;
	float g[6];
	float h[6+5+4+3+2+1];
...
struct FEDepthTracker_KernelParameters {
	FEDepthTracker_CUDA::AccuCell *accu;
	float *depth;
...
__global__ void depthTrackerOneLevel_g_rt_device(FEDepthTracker_KernelParameters para);

// host methods

FEDepthTracker_CUDA::FEDepthTracker_CUDA(Vector2i imgSize, TrackerIterationType *trackingRegime, int noHierarchyLevels, int noICPRunTillLevel,
	float distThresh, float terminationThreshold, const FELowLevelEngine *lowLevelEngine)
	:FEDepthTracker(imgSize, trackingRegime, noHierarchyLevels, noICPRunTillLevel, distThresh, terminationThreshold, lowLevelEngine, MEMORYDEVICE_CUDA)
{
	FESafeCall(cudaMallocHost((void**)&accu_host, sizeof(AccuCell)));
	FESafeCall(cudaMalloc((void**)&accu_device, sizeof(AccuCell)));
}

FEDepthTracker_CUDA::~FEDepthTracker_CUDA(void)
{
	FESafeCall(cudaFreeHost(accu_host));
	FESafeCall(cudaFree(accu_device));
}

int FEDepthTracker_CUDA::ComputeGandH(float &f, float *nabla, float *hessian, Matrix4f approxInvPose)
{
	...
	FESafeCall(cudaMemset(accu_device, 0, sizeof(AccuCell)));

	struct FEDepthTracker_KernelParameters args;
	...
	FESafeCall(cudaMemcpy(accu_host, accu_device, sizeof(AccuCell), cudaMemcpyDeviceToHost));
...
template<bool shortIteration, bool rotationOnly>
__device__ void depthTrackerOneLevel_g_rt_device_main(FEDepthTracker_CUDA::AccuCell *accu, float *depth, Matrix4f approxInvPose, Vector4f *pointsMap,
