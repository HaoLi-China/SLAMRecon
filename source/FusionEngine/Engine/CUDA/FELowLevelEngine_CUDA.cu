...
#include "FELowLevelEngine_CUDA.h"
#include "CUDADefines.h"

#include "../Common/FECLowLevelEngine.h"

using namespace FE;

FELowLevelEngine_CUDA::FELowLevelEngine_CUDA(void) { }
FELowLevelEngine_CUDA::~FELowLevelEngine_CUDA(void) { }
...
void FELowLevelEngine_CUDA::CopyImage(UChar4Image *image_out, const UChar4Image *image_in) const
{
	Vector4u *dest = image_out->GetData(MEMORYDEVICE_CUDA);
	const Vector4u *src = image_in->GetData(MEMORYDEVICE_CUDA);

	FESafeCall(cudaMemcpy(dest, src, image_in->dataSize * sizeof(Vector4u), cudaMemcpyDeviceToDevice));
}

void FELowLevelEngine_CUDA::CopyImage(FloatImage *image_out, const FloatImage *image_in) const
{
	float *dest = image_out->GetData(MEMORYDEVICE_CUDA);
	const float *src = image_in->GetData(MEMORYDEVICE_CUDA);

	FESafeCall(cudaMemcpy(dest, src, image_in->dataSize * sizeof(float), cudaMemcpyDeviceToDevice));
}

void FELowLevelEngine_CUDA::CopyImage(Float4Image *image_out, const Float4Image *image_in) const
{
	Vector4f *dest = image_out->GetData(MEMORYDEVICE_CUDA);
	const Vector4f *src = image_in->GetData(MEMORYDEVICE_CUDA);

	FESafeCall(cudaMemcpy(dest, src, image_in->dataSize * sizeof(Vector4f), cudaMemcpyDeviceToDevice));
}

void FELowLevelEngine_CUDA::FilterSubsample(UChar4Image *image_out, const UChar4Image *image_in) const
{
...
void FELowLevelEngine_CUDA::FilterSubsampleWithHoles(FloatImage *image_out, const FloatImage *image_in) const
{
	Vector2i oldDims = image_in->noDims;
	Vector2i newDims; newDims.x = image_in->noDims.x / 2; newDims.y = image_in->noDims.y / 2;
...
void FELowLevelEngine_CUDA::FilterSubsampleWithHoles(Float4Image *image_out, const Float4Image *image_in) const
{
	Vector2i oldDims = image_in->noDims;
	Vector2i newDims; newDims.x = image_in->noDims.x / 2; newDims.y = image_in->noDims.y / 2;
...
void FELowLevelEngine_CUDA::GradientX(Short4Image *grad_out, const UChar4Image *image_in) const
{
	...
	FESafeCall(cudaMemset(grad, 0, imgSize.x * imgSize.y * sizeof(Vector4s)));

	gradientX_device << <gridSize, blockSize >> >(grad, image, imgSize);
}

void FELowLevelEngine_CUDA::GradientY(Short4Image *grad_out, const UChar4Image *image_in) const
{
	grad_out->ChangeDims(image_in->noDims);
	Vector2i imgSize = image_in->noDims;
