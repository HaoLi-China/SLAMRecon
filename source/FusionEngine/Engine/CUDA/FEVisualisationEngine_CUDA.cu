...
#include "FEVisualisationEngine_CUDA.h"
#include "FECUDAUtils.h"
#include "../Common/FECRepresentationAccess.h"
#include "../Common/FECVisualisationEngine.h"
...
__global__ void buildVisibleList_device(const FEHashEntry *hashTable, /*FEHashCacheState *cacheStates, bool useSwapping,*/ int noTotalEntries,
...
__global__ void projectAndSplitBlocks_device(const FEHashEntry *hashEntries, const int *visibleEntryIDs, int noVisibleEntries,
...
template<class TVoxel, class TIndex>
FEVisualisationEngine_CUDA<TVoxel, TIndex>::FEVisualisationEngine_CUDA(FEScene<TVoxel, TIndex> *scene)
	: FEVisualisationEngine<TVoxel, TIndex>(scene)
{
	FESafeCall(cudaMalloc((void**)&noTotalPoints_device, sizeof(uint)));
}

template<class TVoxel, class TIndex>
FEVisualisationEngine_CUDA<TVoxel, TIndex>::~FEVisualisationEngine_CUDA(void)
{
	FESafeCall(cudaFree(noTotalPoints_device));
}

template<class TVoxel>
FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::FEVisualisationEngine_CUDA(FEScene<TVoxel, FEVoxelBlockHash> *scene)
	: FEVisualisationEngine<TVoxel, FEVoxelBlockHash>(scene)
{
	FESafeCall(cudaMalloc((void**)&renderingBlockList_device, sizeof(RenderingBlock) * MAX_RENDERING_BLOCKS));
	FESafeCall(cudaMalloc((void**)&noTotalBlocks_device, sizeof(uint)));
	FESafeCall(cudaMalloc((void**)&noTotalPoints_device, sizeof(uint)));
	FESafeCall(cudaMalloc((void**)&noVisibleEntries_device, sizeof(uint)));
}

template<class TVoxel>
FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::~FEVisualisationEngine_CUDA(void)
{
	FESafeCall(cudaFree(noTotalPoints_device));
	FESafeCall(cudaFree(noTotalBlocks_device));
	FESafeCall(cudaFree(renderingBlockList_device));
	FESafeCall(cudaFree(noVisibleEntries_device));
}

template<class TVoxel, class TIndex>
FERenderState* FEVisualisationEngine_CUDA<TVoxel, TIndex>::CreateRenderState(const Vector2i & imgSize) const
{
	return new FERenderState(
		imgSize, this->scene->sceneParams->viewFrustum_min, this->scene->sceneParams->viewFrustum_max, MEMORYDEVICE_CUDA
	);
}

template<class TVoxel>
FERenderState_VH* FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::CreateRenderState(const Vector2i & imgSize) const
{
	return new FERenderState_VH(
		FEVoxelBlockHash::noTotalEntries, imgSize, this->scene->sceneParams->viewFrustum_min, this->scene->sceneParams->viewFrustum_max, MEMORYDEVICE_CUDA
	);
}

template<class TVoxel, class TIndex>
void FEVisualisationEngine_CUDA<TVoxel, TIndex>::FindVisibleBlocks(const FEPose *pose, const FEIntrinsics *intrinsics, FERenderState *renderState) const
{
}

template<class TVoxel>
void FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::FindVisibleBlocks(const FEPose *pose, const FEIntrinsics *intrinsics, FERenderState *renderState) const
{
	const FEHashEntry *hashTable = this->scene->index.GetEntries();
	...
	FESafeCall(cudaMemset(noVisibleEntries_device, 0, sizeof(int)));
	...
	FESafeCall(cudaMemcpy(&renderState_vh->noVisibleEntries, noVisibleEntries_device, sizeof(int), cudaMemcpyDeviceToHost));
}

template<class TVoxel, class TIndex>
void FEVisualisationEngine_CUDA<TVoxel, TIndex>::CreateExpectedDepths(const FEPose *pose,	const FEIntrinsics *intrinsics, FERenderState *renderState) const
{
...
template<class TVoxel>
void FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::CreateExpectedDepths(const FEPose *pose, const FEIntrinsics *intrinsics, 
	FERenderState *renderState) const
{
	float voxelSize = this->scene->sceneParams->voxelSize;
	...
	{
		const FEHashEntry *hash_entries = this->scene->index.GetEntries();
		...
		FESafeCall(cudaMemset(noTotalBlocks_device, 0, sizeof(uint)));
	...
	uint noTotalBlocks;
	FESafeCall(cudaMemcpy(&noTotalBlocks, noTotalBlocks_device, sizeof(uint), cudaMemcpyDeviceToHost));
...
template <class TVoxel, class TIndex>
static void GenericRaycast(const FEScene<TVoxel, TIndex> *scene, const Vector2i& imgSize, const Matrix4f& invM, Vector4f projParams, const FERenderState *renderState)
{
...
template<class TVoxel, class TIndex>
static void RenderImage_common(const FEScene<TVoxel, TIndex> *scene, const FEPose *pose, const FEIntrinsics *intrinsics, const FERenderState *renderState,
	UChar4Image *outputImage, PFEVisualisationEngine::RenderImageType type)
{
	...
	if ((type == PFEVisualisationEngine::RENDER_COLOUR_FROM_VOLUME)&&
	    (!TVoxel::hasColorInformation)) type = PFEVisualisationEngine::RENDER_SHADED_GREYSCALE;

	switch (type) {
	case PFEVisualisationEngine::RENDER_COLOUR_FROM_VOLUME:
...
template<class TVoxel, class TIndex>
void CreateICPMaps_common(const FEScene<TVoxel, TIndex> *scene, const FEView *view, FETrackingState *trackingState, FERenderState *renderState)
{
...
template<class TVoxel, class TIndex>
void RenderCurrentView_common(const FEScene<TVoxel, TIndex> *scene, const FEView *view, const Matrix4f &M_d, FERenderState *renderState)
{
	Vector2i imgSize = renderState->raycastResult->noDims;
	Matrix4f invM;
	M_d.inv(invM);
...
template<class TVoxel, class TIndex>
static void ForwardRender_common(const FEScene<TVoxel, TIndex> *scene, const FEView *view, FETrackingState *trackingState, FERenderState *renderState, 
	uint *noTotalPoints_device)
{
	...
	FESafeCall(cudaMemset(noTotalPoints_device, 0, sizeof(uint)));
	...
	FESafeCall(cudaMemcpy(&renderState->noFwdProjMissingPoints, noTotalPoints_device, sizeof(uint), cudaMemcpyDeviceToHost));
...
template<class TVoxel, class TIndex>
void FEVisualisationEngine_CUDA<TVoxel, TIndex>::RenderImage(const FEPose *pose, const FEIntrinsics *intrinsics, const FERenderState *renderState, 
	UChar4Image *outputImage, PFEVisualisationEngine::RenderImageType type) const
{
	RenderImage_common(this->scene, pose, intrinsics, renderState, outputImage, type);
}

template<class TVoxel>
void FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::RenderImage(const FEPose *pose, const FEIntrinsics *intrinsics, 
	const FERenderState *renderState, UChar4Image *outputImage, PFEVisualisationEngine::RenderImageType type) const
{
	RenderImage_common(this->scene, pose, intrinsics, renderState, outputImage, type);
}

template<class TVoxel, class TIndex>
void FEVisualisationEngine_CUDA<TVoxel, TIndex>::FindSurface(const FEPose *pose, const FEIntrinsics *intrinsics, const FERenderState *renderState) const
{
...
template<class TVoxel>
void FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::FindSurface(const FEPose *pose, const FEIntrinsics *intrinsics,
	const FERenderState *renderState) const
{
	GenericRaycast(this->scene, renderState->raycastResult->noDims, pose->GetInvM(), intrinsics->projectionParamsSimple.all, renderState);
}

template<class TVoxel, class TIndex>
void FEVisualisationEngine_CUDA<TVoxel, TIndex>::CreateICPMaps(const FEView *view, FETrackingState *trackingState, 
	FERenderState *renderState) const
{
...
template<class TVoxel>
void FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::CreateICPMaps(const FEView *view, FETrackingState *trackingState, 
	FERenderState *renderState) const
{
	CreateICPMaps_common(this->scene, view, trackingState, renderState);
}
...
template<class TVoxel, class TIndex>
void FEVisualisationEngine_CUDA<TVoxel, TIndex>::RenderCurrentView(const FEView *view, const Matrix4f &M_d, FERenderState *renderState) const
{
	RenderCurrentView_common(this->scene, view, M_d, renderState);
}

template<class TVoxel>
void FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::RenderCurrentView(const FEView *view, const Matrix4f &M_d, FERenderState *renderState) const
{
	RenderCurrentView_common(this->scene, view, M_d, renderState);
}

template<class TVoxel, class TIndex>
void FEVisualisationEngine_CUDA<TVoxel, TIndex>::ForwardRender(const FEView *view, FETrackingState *trackingState, 
	FERenderState *renderState) const
{
...
template<class TVoxel>
void FEVisualisationEngine_CUDA<TVoxel, FEVoxelBlockHash>::ForwardRender(const FEView *view, FETrackingState *trackingState, 
	FERenderState *renderState) const
{
	ForwardRender_common(this->scene, view, trackingState, renderState, this->noTotalPoints_device);
}
	...
	uint *noTotalBlocks)
{
	...
	const FEHashEntry & blockData(hashEntries[visibleEntryIDs[in_offset]]);
...
template class FEVisualisationEngine_CUDA < FEVoxel, FEVoxelIndex > ;
