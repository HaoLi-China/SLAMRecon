...
#ifndef _FE_C_REPRESENTATIONACCESS_H
#define _FE_C_REPRESENTATIONACCESS_H

#include "../../Utils/FELibDefines.h"
#include "../../Utils/FEMathUtils.h"
#include "FECPixelUtils.h"
...
_CPU_AND_GPU_CODE_ inline int findVoxel(const CONSTPTR(FE::FEVoxelBlockHash::IndexData) *voxelIndex, const THREADPTR(Vector3i) & point,
	THREADPTR(bool) &isFound, THREADPTR(FE::FEVoxelBlockHash::IndexCache) & cache)
{
	Vector3i blockPos;
	...
	while (true) 
	{
		FEHashEntry hashEntry = voxelIndex[hashIdx];
...
_CPU_AND_GPU_CODE_ inline int findVoxel(const CONSTPTR(FE::FEVoxelBlockHash::IndexData) *voxelIndex, Vector3i point, THREADPTR(bool) &isFound)
{
	FE::FEVoxelBlockHash::IndexCache cache;
	return findVoxel(voxelIndex, point, isFound, cache);
}

template<class TVoxel>
_CPU_AND_GPU_CODE_ inline TVoxel readVoxel(const CONSTPTR(TVoxel) *voxelData, const CONSTPTR(FE::FEVoxelBlockHash::IndexData) *voxelIndex,
	const THREADPTR(Vector3i) & point, THREADPTR(bool) &isFound, THREADPTR(FE::FEVoxelBlockHash::IndexCache) & cache)
{
...
//template<class TVoxel>
//_CPU_AND_GPU_CODE_ inline TVoxel readVoxel(const CONSTPTR(TVoxel) *voxelData, const CONSTPTR(FE::FEPlainVoxelArray::IndexData) *voxelIndex,
...
//template<class TVoxel>
//_CPU_AND_GPU_CODE_ inline TVoxel readVoxel(const CONSTPTR(TVoxel) *voxelData, const CONSTPTR(FE::FEPlainVoxelArray::IndexData) *voxelIndex,
//	const THREADPTR(Vector3i) & point_orig, THREADPTR(bool) &isFound, THREADPTR(FE::FEPlainVoxelArray::IndexCache) & cache)
//{
...
#endif // _FE_C_REPRESENTATIONACCESS_H