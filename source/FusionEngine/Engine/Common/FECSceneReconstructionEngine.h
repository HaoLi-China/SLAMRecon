...
#ifndef _FE_C_SCENERECONSTRUCTIONENGINE_H
#define _FE_C_SCENERECONSTRUCTIONENGINE_H

#include "../../Utils/FELibDefines.h"
#include "../../Utils/FEMathUtils.h"
#include "FECPixelUtils.h"
#include "FECRepresentationAccess.h"
...
template<class TVoxel>
_CPU_AND_GPU_CODE_ inline float recoverVoxelDepthInfo(DEVICEPTR(TVoxel) &voxel, const THREADPTR(Vector4f) & pt_model, const CONSTPTR(Matrix4f) & M_d,
	const CONSTPTR(Vector4f) & projParams_d, float mu, int maxW, const CONSTPTR(float) *depth, const CONSTPTR(Vector2i) & imgSize)
{
	Vector4f pt_camera; Vector2f pt_image;
	float depth_measure, eta, oldF, newF;
...
_CPU_AND_GPU_CODE_ inline void buildHashAllocAndVisibleTypePP(DEVICEPTR(uchar) *entriesAllocType, DEVICEPTR(uchar) *entriesVisibleType, int x, int y,
	...
	float oneOverVoxelSize, const CONSTPTR(FEHashEntry) *hashTable, float viewFrustum_min, float viewFrustum_max)
{
	...
	for (int i = 0; i < noSteps; i++)
	{
		...
		FEHashEntry hashEntry = hashTable[hashIdx];
...
#endif //_FE_C_SCENERECONSTRUCTIONENGINE_H