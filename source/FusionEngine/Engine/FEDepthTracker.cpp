...
FEDepthTracker::FEDepthTracker(Vector2i imgSize, TrackerIterationType *trackingRegime, int noHierarchyLevels, int noICPRunTillLevel, float distThresh,
	float terminationThreshold, const FELowLevelEngine *lowLevelEngine, MemoryDeviceType memoryType)
{
	viewHierarchy = new FEImageHierarchy<FETemplatedHierarchyLevel<FloatImage> >(imgSize, trackingRegime, noHierarchyLevels, memoryType, true);
	sceneHierarchy = new FEImageHierarchy<FESceneHierarchyLevel>(imgSize, trackingRegime, noHierarchyLevels, memoryType, true);
...
FEDepthTracker::~FEDepthTracker(void) 
{ 
...
void FEDepthTracker::SetEvaluationData(FETrackingState *trackingState, const FEView *view)
{
...
void FEDepthTracker::PrepareForEvaluation()
{
	for (int i = 1; i < viewHierarchy->noLevels; i++)
	{
		FETemplatedHierarchyLevel<FloatImage> *currentLevelView = viewHierarchy->levels[i], *previousLevelView = viewHierarchy->levels[i - 1];
		...
		FESceneHierarchyLevel *currentLevelScene = sceneHierarchy->levels[i], *previousLevelScene = sceneHierarchy->levels[i - 1];
...
void FEDepthTracker::SetEvaluationParams(int levelId)
{
...
void FEDepthTracker::ComputeDelta(float *step, float *nabla, float *hessian, bool shortIteration) const
{
	...
	if (shortIteration)
	{
		...
		Cholesky cholA(smallHessian, 3);
		cholA.Backsub(step, nabla);
	}
	else
	{
		Cholesky cholA(hessian, 6);
		cholA.Backsub(step, nabla);
	}
}

bool FEDepthTracker::HasConverged(float *step) const
{
...
void FEDepthTracker::ApplyDelta(const Matrix4f & para_old, const float *delta, Matrix4f & para_new) const
{
	float step[6];
...
void FEDepthTracker::TrackCamera(FETrackingState *trackingState, const FEView *view)
{
	...
	for (int levelId = viewHierarchy->noLevels - 1; levelId >= noICPLevel; levelId--)
	{
		...
		FEPose lastKnownGoodPose(*(trackingState->pose_d));
		f_old = 1e20f;
		float lambda = 1.0;
