...
void PFEVisualisationEngine::DepthToUchar4(UChar4Image *dst, FloatImage *src)
{
...
void PFEVisualisationEngine::NormalToUchar4(UChar4Image *dst, Float4Image *src)
{
	Vector4u *dest = dst->GetData(MEMORYDEVICE_CPU);
...
void PFEVisualisationEngine::WeightToUchar4(UChar4Image *dst, FloatImage *src)
{
	Vector4u *dest = dst->GetData(MEMORYDEVICE_CPU);
	float *source = src->GetData(MEMORYDEVICE_CPU);
	int dataSize = static_cast<int>(dst->dataSize);
...
template class FE::FEVisualisationEngine<FEVoxel, FEVoxelIndex>;
