...
FEPose::FEPose(void) { this->SetFrom(0, 0, 0, 0, 0, 0); }

FEPose::FEPose(float tx, float ty, float tz, float rx, float ry, float rz) 
{ this->SetFrom(tx, ty, tz, rx, ry, rz); }
FEPose::FEPose(const float pose[6]) { this->SetFrom(pose); }
FEPose::FEPose(const Matrix4f & src) { this->SetM(src); }
FEPose::FEPose(const Vector6f & tangent) { this->SetFrom(tangent); }
FEPose::FEPose(const FEPose & src) { this->SetFrom(&src); }
...
void FEPose::SetFrom(float tx, float ty, float tz, float rx, float ry, float rz)
{
...
void FEPose::SetFrom(const Vector3f &translation, const Vector3f &rotation)
{
...
void FEPose::SetFrom(const Vector6f &tangent)
{
...
void FEPose::SetFrom(const float pose[6])
{
	SetFrom(pose[0], pose[1], pose[2], pose[3], pose[4], pose[5]);
}

void FEPose::SetFrom(const FEPose *pose)
{
...
void FEPose::SetModelViewFromParams()
{
...
void FEPose::SetParamsFromModelView()
{
	Vector3f resultRot;
	Matrix3f R = GetR();
	Vector3f T = GetT();
	...
	FEPose halfrotor(0.0f, 0.0f, 0.0f, resultRot.x * -0.5f, resultRot.y * -0.5f, resultRot.z * -0.5f);
...
FEPose FEPose::exp(const Vector6f& tangent)
{
	return FEPose(tangent);
}

void FEPose::MultiplyWith(const FEPose *pose)
{
	M = M * pose->M;
	this->SetParamsFromModelView();
}

Matrix3f FEPose::GetR(void) const
{
	Matrix3f R;
...
Vector3f FEPose::GetT(void) const
{
	Vector3f T;
...
void FEPose::GetParams(Vector3f &translation, Vector3f &rotation)
{
...
void FEPose::SetM(const Matrix4f & src)
{
	M = src;
	SetParamsFromModelView();
}

void FEPose::SetR(const Matrix3f & R)
{
	M.m[0 + 4*0] = R.m[0 + 3*0]; M.m[1 + 4*0] = R.m[1 + 3*0]; M.m[2 + 4*0] = R.m[2 + 3*0];
	M.m[0 + 4*1] = R.m[0 + 3*1]; M.m[1 + 4*1] = R.m[1 + 3*1]; M.m[2 + 4*1] = R.m[2 + 3*1];
	M.m[0 + 4*2] = R.m[0 + 3*2]; M.m[1 + 4*2] = R.m[1 + 3*2]; M.m[2 + 4*2] = R.m[2 + 3*2];
...
void FEPose::SetT(const Vector3f & t)
{
	M.m[0 + 4*3] = t.v[0]; M.m[1 + 4*3] = t.v[1]; M.m[2 + 4*3] = t.v[2];
...
void FEPose::SetRT(const Matrix3f & R, const Vector3f & t)
{
	M.m[0 + 4*0] = R.m[0 + 3*0]; M.m[1 + 4*0] = R.m[1 + 3*0]; M.m[2 + 4*0] = R.m[2 + 3*0];
	M.m[0 + 4*1] = R.m[0 + 3*1]; M.m[1 + 4*1] = R.m[1 + 3*1]; M.m[2 + 4*1] = R.m[2 + 3*1];
	M.m[0 + 4*2] = R.m[0 + 3*2]; M.m[1 + 4*2] = R.m[1 + 3*2]; M.m[2 + 4*2] = R.m[2 + 3*2];
...
Matrix4f FEPose::GetInvM(void) const
{
	Matrix4f ret;
	M.inv(ret);
	return ret;
...
void FEPose::SetInvM(const Matrix4f & invM)
{
	invM.inv(M);
	SetParamsFromModelView();
}

void FEPose::Coerce(void)
{
	SetParamsFromModelView();
