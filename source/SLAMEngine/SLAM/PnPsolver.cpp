...
namespace SLAMRecon
{
		...
		mnIterations(0), mnBestInliers(0), N(0)
	{
		...
		mvP2D.reserve(F.m_vpMapPoints.size());
		mvSigma2.reserve(F.m_vpMapPoints.size());
		mvP3Dw.reserve(F.m_vpMapPoints.size());
		mvKeyPointIndices.reserve(F.m_vpMapPoints.size());
		mvAllIndices.reserve(F.m_vpMapPoints.size());
		...
		for (size_t i = 0, iend = vpMapPointMatches.size(); i < iend; i++)
		{
			...
			if (pMP)
			{
				if (!pMP->isBad())
				{
					const cv::KeyPoint &kp = F.m_vKeysUn[i];

					mvP2D.push_back(kp.pt);
					mvSigma2.push_back(F.m_pPLevelInfo->m_vLevelSigma2[kp.octave]);
		...
		fu = F.m_pCameraInfo->m_fx;
		fv = F.m_pCameraInfo->m_fy;
		uc = F.m_pCameraInfo->m_cx;
		vc = F.m_pCameraInfo->m_cy;
